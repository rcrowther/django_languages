...
class LanguageField(CharField):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (not self.lang_choices):
            raise ImproperlyConfigured("{}: A 'lang_choices' option must be declared, or passed as __init__ parameter.".format(
    ...
    def to_python(self, value):
        ...
        #return r
        if (isinstance(value, list)):
            return [super(CharField, self).to_python(e) for e in value]
        else:
          return super().to_python(value)

        
    def _validate_code(self, value):
        if not value in self.lang_choices.queryset:
...
class LanguageRelatedField(CharField):
    default_error_messages = {
        'invalid_choice': _('Select a valid choice. %(value)s is not one of the available choices.'),
    }
